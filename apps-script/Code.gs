...
function doPost(e) {
  try {
    ...
    if (action === "recoverManualOutageVip5Transaction20260805") {
      return jsonResponse(recoverManualOutageVip5Transaction20260805_(payload));
    }
...
function recoverManualOutageVip5Transaction20260805_(payload) {
  var request = payload || {};
  ...
  var transactionId = "TRX-MANUAL-20260805-0400-ROOM005";
  var sessionId = "ROOM-005-SESSION-MANUAL-20260805-0400";
  var orderId = "FNB-MANUAL-20260805-0400-ROOM005";
  var startTime = "2026-08-05T04:00:00+07:00";
  var endTime = "2026-08-05T06:00:00+07:00";
  var operationalDate = "2026-08-04";
  var roomId = "ROOM-005";
  var cashierName = "Alfin";
  var customerName = "Tuan VIP 5";
  ...
  if (existingTransaction) {
    return {
      ...
      message: "Transaksi manual VIP 5 sudah pernah dipulihkan.",
  ...
  if (!room) {
    return { ok: false, success: false, error: "Master Ruangan 5 - VIP 5 tidak ditemukan." };
  }

  var ratePerHour = Number(room.rate_per_hour) || 0;
  if (ratePerHour <= 0) {
    return { ok: false, success: false, error: "Tarif master Ruangan 5 - VIP 5 tidak valid." };
  }
  ...
  Object.keys(menuQuantities).forEach(function (menuId) {
    ...
    if (price <= 0) {
      validationErrors.push("Harga master tidak valid: " + menuItem.menu_name);
      return;
    }
  ...
  var roomTotal = ratePerHour * (durationMinutes / 60);
  var fnbTotal = orderItems.reduce(function (sum, item) {
    return sum + item.subtotal;
  }, 0);
  var grandTotal = roomTotal + fnbTotal;
  var preview = {
  ...
  if (!shouldExecute) {
    return {
      ...
      success: true,
      dry_run: true,
      message: "Dry run recovery VIP 5 siap dieksekusi.",
      preview: preview,
    };
  }

  if (confirmation !== "RECOVER-VIP5-20260805-0400") {
    return {
      ok: false,
      success: false,
  ...
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) {
    return createLockBusyResponse_("Sistem sedang memproses transaksi lain. Coba lagi.");
  }
  ...
  try {
    ...
    var partialSession = readSheetAsObjectsOrEmpty_("RoomSessions").some(function (session) {
      return String(session.session_id || "").trim() === sessionId;
    });
    var partialOrder = readSheetAsObjectsOrEmpty_("FnbOrders").some(function (order) {
      return String(order.order_id || "").trim() === orderId;
    });
    if (partialSession || partialOrder) {
      return {
        ok: false,
        success: false,
        error: "Ditemukan data recovery parsial. Hentikan dan periksa sebelum mencoba lagi.",
      };
    }

    sheets.RoomSessions = ensureRoomSessionsSheet_();
    sheets.FnbOrders = ensureFnbOrdersSheetColumns_();
    sheets.FnbOrderItems = ensureFnbOrderItemsSheet_();
    sheets.Transactions = ensureTransactionsSheetColumns_();
    sheets.StockMovements = ensureStockMovementsSheet_();
    Object.keys(sheets).forEach(function (sheetName) {
      initialRows[sheetName] = sheets[sheetName].getLastRow();
    });

    var stockPlan = calculateStockRequirementsFromFnbOrders_([
    ...
    stockPlan.requirements.forEach(function (requirement) {
      ...
      inventorySnapshots.push({
        ...
        stock_qty_column: inventory.header_map.stock_qty,
        updated_at_column: inventory.header_map.updated_at || 0,
        stock_qty: inventory.sheet.getRange(
          inventory.row_number,
          inventory.header_map.stock_qty
        ).getValue(),
        updated_at: inventory.header_map.updated_at
          ? inventory.sheet.getRange(inventory.row_number, inventory.header_map.updated_at).getValue()
          : "",
      });
    });
    ...
    var order = {
      ...
      cashier_name: cashierName,
      note: "Recovery transaksi manual karena mati listrik; tanpa LC.",
      customer_name: customerName,
      general_bill_id: "",
      billed_transaction_id: transactionId,
      idempotency_key: "manual-outage-20260805-0400-room005",
      created_at: endTime,
      updated_at: endTime,
    };
    var session = {
      ...
      note: "Recovery transaksi manual karena mati listrik; waktu 04.00-06.00 dikonfirmasi owner.",
      customer_name: customerName,
      package_id: "",
      prepayment_transaction_id: "",
      lc_ids: "",
    ...
    appendMasterDataAuditLog_({
      ...
      entity_name: room.room_name,
      action_type: "recover_manual_outage_transaction",
      old_value: {},
      new_value: preview,
      changed_by: "Codex atas konfirmasi Owner",
      note: "Backfill nota mati listrik VIP 5 tanggal 5 Agustus 2026 pukul 04.00-06.00.",
      result: "success",
    });
    ...
    return {
      ok: true,
      success: true,
      message: "Transaksi manual VIP 5 berhasil dipulihkan.",
    ...
    inventorySnapshots.forEach(function (snapshot) {
      snapshot.sheet.getRange(snapshot.row_number, snapshot.stock_qty_column).setValue(snapshot.stock_qty);
      if (snapshot.updated_at_column) {
        snapshot.sheet.getRange(snapshot.row_number, snapshot.updated_at_column).setValue(snapshot.updated_at);
      }
    });

    Object.keys(sheets).forEach(function (sheetName) {
      var sheet = sheets[sheetName];
      var initialLastRow = initialRows[sheetName];
      var appendedRows = sheet.getLastRow() - initialLastRow;
      if (appendedRows > 0) {
        sheet.deleteRows(initialLastRow + 1, appendedRows);
      }
    });
    ...
    return {
      ok: false,
      success: false,
      error: "Recovery dibatalkan dan perubahan dikembalikan: "
