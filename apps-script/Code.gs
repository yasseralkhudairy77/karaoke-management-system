...
function doPost(e) {
  try {
    ...
    if (action === "correctLcDurationTransaction20260806223846618") {
      return jsonResponse(correctLcDurationTransaction20260806223846618_(payload));
    }
...
function correctLcDurationTransaction20260806223846618_(payload) {
  var request = payload || {};
  var shouldExecute = request.execute === true
    || String(request.execute || "").trim().toLowerCase() === "true";
  var confirmation = String(request.confirmation || "").trim().toUpperCase();
  var transactionId = "TRX-20260806223846618-F417DD42";
  var sessionId = "ROOM-001-SESSION-20260806204817-216";
  var targetLcTotal = 500000;
  var targetDurationMinutes = 120;
  var targetRate = 250000;
  var targetAssignments = [
    { lc_id: "LC-040", duration_minutes: targetDurationMinutes },
    { lc_id: "LC-041", duration_minutes: targetDurationMinutes },
  ];
  ...
  if (!transactionRow) {
    return { ok: false, success: false, error: "Transaksi target tidak ditemukan." };
  }

  var transaction = getRowObject_(transactionsSheet, transactionHeaders, transactionRow);
  var targetGrandTotal = (Number(transaction.room_total) || 0)
  ...
  var workLogs = readSheetAsObjects_("LcWorkLogs");
  var targetLogs = targetAssignments.map(function (assignment) {
    var match = null;
    var rowNumber = 0;

    for (var index = 0; index < workLogs.length; index++) {
      var log = workLogs[index];
      if (
        String(log.session_id || "").trim() === sessionId
        && String(log.lc_id || "").trim() === assignment.lc_id
      ) {
        match = log;
        rowNumber = index + 2;
        break;
  ...
  var missingLcIds = targetLogs.filter(function (entry) {
    return !entry.log;
  }).map(function (entry) {
    return entry.assignment.lc_id;
  });
  ...
  if (missingLcIds.length > 0) {
    return {
      ok: false,
      success: false,
      error: "Work log LC target tidak lengkap: " + missingLcIds.join(", "),
    };
  }

  var sessionsSheet = ensureRoomSessionsSheet_();
  var sessionHeaders = getHeaderMap_(sessionsSheet);
  var sessionRow = findRowByValue_(sessionsSheet, sessionHeaders, "session_id", sessionId);
  if (!sessionRow) {
    return { ok: false, success: false, error: "Room session target tidak ditemukan." };
  }

  var preview = {
    transaction_id: transactionId,
    session_id: sessionId,
    payment_status: String(transaction.payment_status || "").trim().toLowerCase(),
    old_lc_total: Number(transaction.lc_total) || 0,
    new_lc_total: targetLcTotal,
    old_grand_total: Number(transaction.grand_total) || 0,
    new_grand_total: targetGrandTotal,
    lc_logs: targetLogs.map(function (entry) {
      return {
        log_id: entry.log.log_id || "",
        lc_id: entry.log.lc_id || "",
        lc_name: entry.log.lc_name || "",
        old_duration_minutes: Number(entry.log.duration_minutes) || 0,
        new_duration_minutes: targetDurationMinutes,
        old_rate: Number(entry.log.rate) || 0,
        new_rate: targetRate,
      };
    }),
  };

  var alreadyCorrected = preview.old_lc_total === targetLcTotal
    && preview.old_grand_total === targetGrandTotal
    && targetLogs.every(function (entry) {
      return Number(entry.log.duration_minutes) === targetDurationMinutes
        && Number(entry.log.rate) === targetRate;
    });
  ...
  if (alreadyCorrected) {
    return {
      ...
      message: "Durasi dan biaya LC transaksi sudah benar.",
      preview: preview,
    };
  }
  ...
  if (!shouldExecute) {
    return {
      ...
      success: true,
      dry_run: true,
      message: "Dry run koreksi durasi LC siap dieksekusi.",
      preview: preview,
    };
  }

  if (confirmation !== "CORRECT-LC-20260806223846618") {
    return { ok: false, success: false, error: "Konfirmasi koreksi tidak cocok." };
  }
  ...
  try {
    transactionsSheet.getRange(transactionRow, transactionHeaders.lc_total).setValue(targetLcTotal);
    transactionsSheet.getRange(transactionRow, transactionHeaders.grand_total).setValue(targetGrandTotal);

    targetLogs.forEach(function (entry) {
      workLogsSheet.getRange(entry.row_number, workLogHeaders.duration_minutes).setValue(targetDurationMinutes);
      workLogsSheet.getRange(entry.row_number, workLogHeaders.rate).setValue(targetRate);
      workLogsSheet.getRange(entry.row_number, workLogHeaders.rate_per_hour).setValue(125000);
    });

    setRowValues_(sessionsSheet, sessionHeaders, sessionRow, {
      lc_ids: "LC-040,LC-041",
      lc_assignments: serializeLcAssignments_(targetAssignments),
      updated_at: toJakartaIsoString_(new Date()),
    });

    try {
      CacheService.getScriptCache().removeAll([
        "lc-work-reports-v2:today:2026-08-06:2026-08-06",
        "lc-work-reports-v2:custom:2026-08-06:2026-08-06",
        "lc-work-reports-v2:all:all:all",
      ]);
    } catch (cacheError) {
      Logger.log("Gagal membersihkan cache laporan LC: " + cacheError.message);
    }

    appendMasterDataAuditLog_({
      entity_type: "transaction",
      entity_id: transactionId,
      entity_name: transaction.room_name || "",
      action_type: "correct_lc_duration",
      old_value: preview,
      new_value: {
        lc_total: targetLcTotal,
        grand_total: targetGrandTotal,
        lc_assignments: targetAssignments,
      },
      changed_by: "Codex atas konfirmasi Owner",
      note: "Koreksi Dewi dan Dila menjadi masing-masing 120 menit.",
      result: "success",
    });
    ...
    return {
      ok: true,
      success: true,
      message: "Durasi dan biaya dua LC berhasil dikoreksi.",
      transaction: getRowObject_(transactionsSheet, transactionHeaders, transactionRow),
      lc_logs: targetLogs.map(function (entry) {
        return {
          log_id: entry.log.log_id || "",
          lc_id: entry.log.lc_id || "",
