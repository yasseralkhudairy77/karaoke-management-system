...
function getPushConfig() {
  return {
    ...
    period: process.env.OWNER_MIRROR_PERIOD || 'today'
  };
}

async function pushOwnerMirrorSnapshot() {
  ...
  isPushRunning = true;
  try {
    const snapshot = await buildOwnerMirrorSnapshot({ period: config.period });
...
function startOwnerMirrorPushWorker(intervalMs = 60000) {
  ...
  console.log(`Owner Mirror Push Worker started (Polling every ${intervalMs}ms).`);
