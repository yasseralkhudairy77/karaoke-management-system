...
function getDefaultLcDurationMinutes(room) {
  const bookedDuration = Math.round(Number(room?.booked_duration_minutes) || 0);
  return bookedDuration > 0 ? bookedDuration : 60;
}
...
let previousLcReminderRoomIds = new Set();
...
let lcDurationEditor = null;
let isLoadingLcDurationEditor = false;
let isSavingLcDurationEditor = false;
...
let activeSessionPackageRoomId = "";
...
const ROOM_LC_REMINDER_MS = 15 * 60 * 1000;
...
function hasSelectedSessionLc(room) {
  return String(room?.lc_ids || "")
    .split(",")
    .map((lcId) => lcId.trim())
    .some((lcId) => lcId && lcId.toUpperCase() !== "PENDING");
}

function shouldRemindSessionLc(room, remainingMs = getRoomRemainingTime(room)) {
  return normalizeRoomStatus(room?.status) === "occupied"
    && Number.isFinite(remainingMs)
    && remainingMs <= ROOM_LC_REMINDER_MS
    && !hasSelectedSessionLc(room);
}

function getRoomsLcReminderRoomIds() {
  return rooms
    .filter((room) => shouldRemindSessionLc(room))
    .map((room) => room.room_id);
}

function getRoomsTimeWarningCount() {
  return new Set([
    ...getRoomsTimeWarningRoomIds(),
    ...getRoomsLcReminderRoomIds(),
  ]).size;
}
...
function updateRoomsTimeWarningAwareness({ playSound = false } = {}) {
  ...
  const lcReminderRoomIds = new Set(getRoomsLcReminderRoomIds());
  const hasNewWarningRoom = Array.from(warningRoomIds).some(
    (roomId) => !previousWarningRoomIds.has(roomId)
  );
  const newLcReminderRoomIds = Array.from(lcReminderRoomIds).filter(
    (roomId) => !previousLcReminderRoomIds.has(roomId)
  );

  updateRoomsTabWarningBadge(new Set([...warningRoomIds, ...lcReminderRoomIds]).size);
  ...
  if (!roomWarningStateInitialized) {
    ...
    previousLcReminderRoomIds = lcReminderRoomIds;
    roomWarningStateInitialized = true;
    return;
  }

  if (playSound && (hasNewWarningRoom || newLcReminderRoomIds.length > 0)) {
    playRoomWarningSound();
  }

  if (newLcReminderRoomIds.length > 0) {
    const roomNames = newLcReminderRoomIds.map((roomId) => (
      rooms.find((room) => room.room_id === roomId)?.room_name || roomId
    ));
    showInlineNotice(`Pengingat LC: jangan lupa pilih LC untuk ${roomNames.join(", ")}.`, "warning");
  }
...
function updateRoomLcReminder(reminder, room, remainingMs = getRoomRemainingTime(room)) {
  if (!reminder) {
    return;
  }

  reminder.hidden = !shouldRemindSessionLc(room, remainingMs);
}
...
async function openLcDurationEditor(transactionId) {
  ...
  try {
    ...
    const data = await response.json();
    if (!data || data.ok !== true) {
      throw new Error(data?.error || "Detail durasi LC tidak dapat dimuat.");
    }
    ...
    lcDurationEditor = {
      ...
      loading: false,
      details: data,
      durations: (data.lc_logs || []).reduce((result, log) => {
        result[log.lc_id] = Number(log.duration_minutes) || 60;
        return result;
      }, {}),
    };
  } catch (error) {
    lcDurationEditor = {
      ...lcDurationEditor,
      loading: false,
      error: error.message || "Detail durasi LC tidak dapat dimuat.",
    };
  } finally {
    isLoadingLcDurationEditor = false;
    renderRooms();
  }
}
...
function setLcDurationEditorDuration(lcId, value) {
  ...
  lcDurationEditor.durations = {
    ...lcDurationEditor.durations,
    [lcId]: Math.round(Number(value)) || 60,
  };
  renderRooms();
}

function updateLcDurationEditorField(field, value) {
  if (!lcDurationEditor || isSavingLcDurationEditor) {
    return;
  }

  lcDurationEditor[field] = value;
...
function mergeUpdatedTransactionIntoState(updatedTransaction) {
...
async function saveLcDurationEditor() {
  const editor = lcDurationEditor;
  const details = editor?.details;
  const preview = getLcDurationEditorPreview();

  if (!editor || !details || isSavingLcDurationEditor) {
    return;
  }
  if (!details.can_edit) {
    editor.error = details.blocked_reason || "Durasi LC tidak dapat diedit.";
    renderRooms();
    return;
  }
  if (!preview.changed) {
    editor.error = "Belum ada durasi yang diubah.";
    renderRooms();
    return;
  }
  if (String(editor.reason || "").trim().length < 3) {
    editor.error = "Alasan perubahan minimal 3 karakter.";
    renderRooms();
    return;
  }
  ...
  try {
    ...
    isSavingLcDurationEditor = false;
    renderRooms();
  }
}

function createLcDurationEditorElement() {
  if (!lcDurationEditor) {
    return document.createDocumentFragment();
  }
  ...
  const details = editor.details;
  const overlay = document.createElement("section");
  overlay.className = "lc-duration-editor-overlay";
  overlay.setAttribute("role", "dialog");
  overlay.setAttribute("aria-modal", "true");
  overlay.setAttribute("aria-labelledby", "lc-duration-editor-title");

  const dialog = document.createElement("div");
  dialog.className = "lc-duration-editor-dialog";

  const header = document.createElement("div");
  header.className = "lc-duration-editor-header";
  ...
  subtitle.textContent = details
    ? `${details.room_name || details.room_id} · ${details.transaction_id}`
    : editor.transaction_id;
  heading.append(title, subtitle);

  const closeButton = document.createElement("button");
  closeButton.className = "lc-duration-editor-close";
  closeButton.type = "button";
  closeButton.dataset.action = "close-lc-duration-editor";
  closeButton.disabled = isSavingLcDurationEditor;
  closeButton.setAttribute("aria-label", "Tutup editor durasi LC");
  ...
  if (editor.loading) {
    dialog.appendChild(createStateMessage("Memuat rincian LC..."));
    overlay.appendChild(dialog);
    return overlay;
  }

  if (editor.error) {
    dialog.appendChild(createStateMessage(editor.error, "error"));
  }
  ...
  if (!details) {
    ...
    close.className = "billing-summary-close";
    close.type = "button";
    close.dataset.action = "close-lc-duration-editor";
  ...
  (details.lc_logs || []).forEach((log) => {
    ...
    identity.className = "lc-duration-editor-identity";
    const name = document.createElement("strong");
    name.textContent = log.lc_name || log.lc_id;
    const rate = document.createElement("span");
    rate.textContent = `${formatCurrency(log.rate_per_hour)} / jam`;
    identity.append(name, rate);

    const durationField = document.createElement("label");
    durationField.className = "lc-duration-editor-field";
    const durationLabel = document.createElement("span");
    durationLabel.textContent = "Durasi";
    const select = document.createElement("select");
    select.className = "lc-duration-editor-select";
    select.disabled = !details.can_edit || isSavingLcDurationEditor;
    const currentDuration = Number(editor.durations?.[log.lc_id]) || Number(log.duration_minutes) || 60;
    for (let minutes = 30; minutes <= 720; minutes += 30) {
      const option = document.createElement("option");
      option.value = String(minutes);
      option.textContent = formatLcDurationOptionLabel(minutes);
      option.selected = minutes === currentDuration;
      select.appendChild(option);
    }
    select.addEventListener("change", (event) => {
      setLcDurationEditorDuration(log.lc_id, event.target.value);
    });
  ...
  totals.className = "lc-duration-editor-totals";
  [
    ["Total LC Lama", formatCurrency(preview.oldLcTotal)],
    ["Total LC Baru", formatCurrency(preview.newLcTotal)],
    ["Grand Total Baru", formatCurrency(preview.newGrandTotal), "total"],
  ].forEach(([labelText, valueText, modifier]) => {
    const row = document.createElement("div");
    row.className = modifier
      ? "lc-duration-editor-total-row total"
      : "lc-duration-editor-total-row";
  ...
  reasonField.className = "lc-duration-editor-reason";
  ...
  reasonInput.addEventListener("input", (event) => {
    updateLcDurationEditorField("reason", event.target.value);
  });
  ...
  actions.className = "lc-duration-editor-actions";
  const cancelButton = document.createElement("button");
  cancelButton.className = "billing-summary-close";
  cancelButton.type = "button";
  cancelButton.dataset.action = "close-lc-duration-editor";
  cancelButton.disabled = isSavingLcDurationEditor;
  cancelButton.textContent = "Batal";

  const saveButton = document.createElement("button");
  saveButton.className = "lc-duration-editor-save";
  saveButton.type = "button";
  saveButton.dataset.action = "save-lc-duration-editor";
...
function createBillingSummaryElement(transaction) {
  ...
  if (canOpenLcDurationEditor(transaction)) {
    const editLcButton = document.createElement("button");
    editLcButton.className = "billing-summary-close";
...
function createRoomBookingInfoElement(room) {
  ...
  lcReminder.dataset.role = "lc-reminder";
  lcReminder.textContent = "Pengingat 15 menit: jangan lupa pilih LC.";
  updateRoomLcReminder(lcReminder, room, timeState.remainingMs);
  info.appendChild(lcReminder);
...
function createActiveSessionPackageElement(room) {
  const panel = document.createElement("div");
  panel.className = "extend-selection";

  const title = document.createElement("p");
  title.className = "extend-selection-title";
  title.textContent = `Ubah paket untuk ${room.room_name}`;

  const currentInfo = document.createElement("div");
  currentInfo.className = "extend-note-field";
  currentInfo.style.backgroundColor = "rgba(124, 58, 237, 0.1)";
  currentInfo.style.padding = "8px 12px";
  currentInfo.style.borderRadius = "var(--radius-sm)";
  currentInfo.style.border = "1px dashed rgba(124, 58, 237, 0.3)";
  currentInfo.style.fontSize = "12px";
  currentInfo.style.color = "#d8b4fe";
  currentInfo.textContent = `Paket saat ini: ${room.package_id || room.package_name ? getRoomPriceLabel(room) : "Tanpa Paket"}`;

  const packageField = document.createElement("div");
  packageField.className = "extend-note-field";

  const packageLabel = document.createElement("label");
  packageLabel.className = "extend-note-label";
  packageLabel.textContent = "Paket baru";

  const packageSelect = document.createElement("select");
  packageSelect.className = "duration-payment-select";
  packageSelect.style.width = "100%";
  packageSelect.dataset.action = "update-active-session-package-field";
  packageSelect.dataset.field = "package_id";
  packageSelect.disabled = isUpdatingActiveSessionPackage;

  const noneOption = document.createElement("option");
  noneOption.value = "";
  noneOption.textContent = `Tanpa Paket - pakai tarif room (${formatCurrency(room.rate_per_hour)}/jam)`;
  noneOption.selected = !activeSessionPackageSelection;
  packageSelect.appendChild(noneOption);

  packages
    .filter((pkg) => String(pkg.status || "active").trim().toLowerCase() === "active")
    .forEach((pkg) => {
      const option = document.createElement("option");
      ...
      option.selected = pkg.package_id === activeSessionPackageSelection;
      packageSelect.appendChild(option);
    });

  packageField.append(packageLabel, packageSelect);

  const selectedPackage = packages.find((pkg) => pkg.package_id === activeSessionPackageSelection);
  const preview = document.createElement("div");
  preview.className = "extend-note-field";
  preview.style.fontSize = "12px";
  preview.style.color = "rgba(255,255,255,0.72)";
  preview.textContent = selectedPackage
    ? `Tagihan room/paket saat checkout akan memakai ${selectedPackage.package_name}: ${formatCurrency(selectedPackage.selling_price)}. F&B dan LC tetap.`
    : "Tagihan room saat checkout akan kembali memakai tarif room dan durasi sesi berjalan. F&B dan LC tetap.";
  ...
  reasonLabel.textContent = "Alasan ubah paket (wajib)";

  const reasonInput = document.createElement("input");
  reasonInput.className = "extend-note-input";
  reasonInput.type = "text";
  reasonInput.placeholder = "Contoh: Info paket terlambat dari waiter";
  reasonInput.dataset.action = "update-active-session-package-field";
  reasonInput.dataset.field = "reason";
  reasonInput.value = activeSessionPackageReason;
  reasonInput.disabled = isUpdatingActiveSessionPackage;
  ...
  saveButton.type = "button";
  saveButton.dataset.action = "save-active-session-package";
  saveButton.dataset.roomId = room.room_id;
  saveButton.disabled = isUpdatingActiveSessionPackage;
  saveButton.textContent = isUpdatingActiveSessionPackage ? "Menyimpan..." : "Simpan Ubah Paket";

  const cancelButton = document.createElement("button");
  cancelButton.className = "extend-cancel-button";
  cancelButton.type = "button";
  cancelButton.dataset.action = "cancel-active-session-package";
  cancelButton.disabled = isUpdatingActiveSessionPackage;
  cancelButton.textContent = "Batal";
...
function updateRunningTimers() {
  ...
  occupiedCards.forEach((card) => {
    ...
    const lcReminder = card.querySelector("[data-role='lc-reminder']");
    const room = rooms.find((item) => item.room_id === card.dataset.roomId);
    ...
    if (timer.dataset.scheduledEndTime) {
      ...
      updateRoomLcReminder(lcReminder, room, remainingTime);
    ...
    updateRoomLcReminder(lcReminder, room, null);
...
function showActiveSessionPackageSelection(roomId) {
  const room = rooms.find((item) => item.room_id === roomId);
  extendSelectionRoomId = "";
  lcSelectionRoomId = "";
  moveRoomSourceId = "";
...
function cancelActiveSessionPackageSelection() {
  activeSessionPackageRoomId = "";
  activeSessionPackageSelection = "";
  activeSessionPackageReason = "";
  renderRooms();
}

function updateActiveSessionPackageField(field, value) {
  if (field === "package_id") {
    activeSessionPackageSelection = value;
    renderRooms();
    return;
  }

  if (field === "reason") {
    activeSessionPackageReason = value;
  }
}

async function saveActiveSessionPackage(roomId) {
  if (!API_BASE_URL.trim()) {
    showInlineNotice("API belum dikonfigurasi. Isi URL server dulu di config.js.", "error");
    return;
  }

  const reason = activeSessionPackageReason.trim();
  if (!reason) {
    showInlineNotice("Alasan ubah paket wajib diisi.", "error");
    return;
  }
  ...
  isUpdatingActiveSessionPackage = true;
  setActionButtonsDisabled(true);
  renderRooms();

  try {
    const data = await postApiAction({
      action: "updateActiveSessionPackage",
      room_id: roomId,
      package_id: activeSessionPackageSelection,
      reason,
      cashier_name: getLoggedInOperatorName(),
    });

    if (!data || data.ok !== true) {
      throw new Error(data?.error || "Gagal mengubah paket sesi.");
    }

    showInlineNotice(data.message || "Paket sesi berhasil diubah.");
    activeSessionPackageRoomId = "";
    activeSessionPackageSelection = "";
    activeSessionPackageReason = "";
    await loadRooms();
  } catch (error) {
    showInlineNotice(error.message || "Gagal mengubah paket sesi.", "error");
  } finally {
    isUpdatingActiveSessionPackage = false;
...
async function handleRoomAction(event) {
  ...
  if (action === "open-lc-duration-editor") {
    await openLcDurationEditor(button.dataset.transactionId || "");
    return;
  }

  if (action === "close-lc-duration-editor") {
    closeLcDurationEditor();
    return;
  }

  if (action === "save-lc-duration-editor") {
    await saveLcDurationEditor();
    return;
  }
  ...
  if (action === "prepare-room-session-custom-duration") {
    ...
    const roomId = button.dataset.roomId || "";
    await prepareRoomSession(roomId, selectedDuration, customerNameInput);
    customerNameInput = "";
    delete selectedLcIdsForRoom[roomId];
    delete selectedLcDurationsForRoom[roomId];
    return;
  ...
  if (action === "show-active-session-package") {
    showActiveSessionPackageSelection(button.dataset.roomId || roomId || "");
    return;
  }

  if (action === "cancel-active-session-package") {
    cancelActiveSessionPackageSelection();
    return;
  }

  if (action === "save-active-session-package") {
    await saveActiveSessionPackage(button.dataset.roomId || roomId || "");
    return;
  }
...
function handleDashboardChange(event) {
  ...
  const activeSessionPackageField = event.target.closest("[data-action='update-active-session-package-field']");
